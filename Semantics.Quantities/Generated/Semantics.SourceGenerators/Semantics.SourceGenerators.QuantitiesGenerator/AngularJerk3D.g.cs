	...
	where T : struct, INumber<T>
{
	...
	public static AngularJerk3D<T> Zero => new() { X = T.Zero, Y = T.Zero, Z = T.Zero };

	/// <summary>Gets a vector with all components set to one.</summary>
	public static AngularJerk3D<T> One => new() { X = T.One, Y = T.One, Z = T.One };

	/// <summary>Gets the unit vector for the X-axis.</summary>
	public static AngularJerk3D<T> UnitX => new() { X = T.One, Y = T.Zero, Z = T.Zero };

	/// <summary>Gets the unit vector for the Y-axis.</summary>
	public static AngularJerk3D<T> UnitY => new() { X = T.Zero, Y = T.One, Z = T.Zero };
	...
	/// <summary>Creates a <see cref="AngularJerk3D{T}"/> from components in RadianPerSecondCubed.</summary>
	/// <param name="x">The X component, in RadianPerSecondCubed.</param>
	/// <param name="y">The Y component, in RadianPerSecondCubed.</param>
	/// <param name="z">The Z component, in RadianPerSecondCubed.</param>
	/// <returns>A new <see cref="AngularJerk3D{T}"/> storing the SI-base equivalent.</returns>
	public static AngularJerk3D<T> FromRadianPerSecondCubed(T x, T y, T z) => new() { X = x, Y = y, Z = z };

	/// <summary>Converts this vector's SI-base components to <paramref name="unit"/>.</summary>
	/// <remarks>Returns bare components rather than a <see cref="AngularJerk3D{T}"/>, because a vector that is not in base units cannot be one.</remarks>
	/// <param name="unit">The dimensionally-compatible target unit.</param>
	/// <returns>The components expressed in <paramref name="unit"/>.</returns>
	public (T X, T Y, T Z) In(global::ktsu.Semantics.Quantities.IAngularJerkUnit unit) => (unit.FromBase(X), unit.FromBase(Y), unit.FromBase(Z));
	...
	public T Dot(AngularJerk3D<T> other) => (X * other.X) + (Y * other.Y) + (Z * other.Z);

	/// <summary>Calculates the cross product of two vectors.</summary>
	public AngularJerk3D<T> Cross(AngularJerk3D<T> other)
	{
		return new() { X = (Y * other.Z) - (Z * other.Y), Y = (Z * other.X) - (X * other.Z), Z = (X * other.Y) - (Y * other.X) };
	}

	/// <summary>Calculates the distance between two vectors.</summary>
	public T Distance(AngularJerk3D<T> other)
	{
		T dX = X - other.X;
		T dY = Y - other.Y;
		T dZ = Z - other.Z;
	...
	public T DistanceSquared(AngularJerk3D<T> other)
	{
		T dX = X - other.X;
		T dY = Y - other.Y;
		T dZ = Z - other.Z;
	...
	public AngularJerk3D<T> Normalize()
	{
		T len = Length();
	...
	public static AngularJerk3D<T> operator +(AngularJerk3D<T> left, AngularJerk3D<T> right) => new() { X = left.X + right.X, Y = left.Y + right.Y, Z = left.Z + right.Z };

	/// <summary>Subtracts two vectors.</summary>
	public static AngularJerk3D<T> operator -(AngularJerk3D<T> left, AngularJerk3D<T> right) => new() { X = left.X - right.X, Y = left.Y - right.Y, Z = left.Z - right.Z };

	/// <summary>Multiplies a vector by a scalar.</summary>
	public static AngularJerk3D<T> operator *(AngularJerk3D<T> vector, T scalar) => new() { X = vector.X * scalar, Y = vector.Y * scalar, Z = vector.Z * scalar };

	/// <summary>Multiplies a scalar by a vector.</summary>
	public static AngularJerk3D<T> operator *(T scalar, AngularJerk3D<T> vector) => new() { X = scalar * vector.X, Y = scalar * vector.Y, Z = scalar * vector.Z };

	/// <summary>Divides a vector by a scalar.</summary>
	public static AngularJerk3D<T> operator /(AngularJerk3D<T> vector, T scalar) => new() { X = vector.X / scalar, Y = vector.Y / scalar, Z = vector.Z / scalar };

	/// <summary>Negates a vector.</summary>
	public static AngularJerk3D<T> operator -(AngularJerk3D<T> vector) => new() { X = -vector.X, Y = -vector.Y, Z = -vector.Z };
	/// <summary>AngularJerk3D * Duration = AngularAcceleration3D.</summary>
	public static AngularAcceleration3D<T> operator *(AngularJerk3D<T> left, Duration<T> right) => new() { X = left.X * right.Value, Y = left.Y * right.Value, Z = left.Z * right.Value };
