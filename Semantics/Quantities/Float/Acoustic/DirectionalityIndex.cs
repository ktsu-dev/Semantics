...
public sealed record DirectionalityIndex : Generic.DirectionalityIndex<float>
{
	...
	public static new DirectionalityIndex FromDecibels(float decibels) => (DirectionalityIndex)Create(decibels);
	...
	public static new DirectionalityIndex FromDirectivityFactor(float directivityFactor) => (DirectionalityIndex)Create(10.0f * (float)Math.Log10(directivityFactor));
}
