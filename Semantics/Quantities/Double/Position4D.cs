...
/// <summary>
/// Represents a 4D spacetime position vector with double precision (x, y, z, ct).
/// The fourth component represents time multiplied by the speed of light for dimensional consistency.
/// </summary>
public sealed record Position4D : Generic.Position4D<double>
{
	...
	public static Position4D Create(Vector4d value) => new() { Value = value };
	...
	/// <returns>A new instance of the quantity.</returns>
	public static Position4D Create(double x, double y, double z, double ct) => Create(new Vector4d(x, y, z, ct));
	...
	public static Position4D FromSpacetime(double x, double y, double z, double time)
	{
	...
	/// <param name="time">The time coordinate in seconds.</param>
	/// <returns>A new Position4D instance.</returns>
	public static Position4D FromSpacetime(Position3D position3D, double time)
	{
	...
	public double MinkowskiDot(Position4D other)
	{
		ArgumentNullException.ThrowIfNull(other);
	...
	public override string ToString() => $"({X:F6}, {Y:F6}, {Z:F6}, {CT:F6}) {Dimension.BaseUnit.Symbol}";
}