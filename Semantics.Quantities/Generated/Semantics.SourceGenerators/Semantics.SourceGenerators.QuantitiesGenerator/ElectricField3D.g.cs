	...
	where T : struct, INumber<T>
{
	...
	public static ElectricField3D<T> Zero => new() { X = T.Zero, Y = T.Zero, Z = T.Zero };

	/// <summary>Gets a vector with all components set to one.</summary>
	public static ElectricField3D<T> One => new() { X = T.One, Y = T.One, Z = T.One };

	/// <summary>Gets the unit vector for the X-axis.</summary>
	public static ElectricField3D<T> UnitX => new() { X = T.One, Y = T.Zero, Z = T.Zero };

	/// <summary>Gets the unit vector for the Y-axis.</summary>
	public static ElectricField3D<T> UnitY => new() { X = T.Zero, Y = T.One, Z = T.Zero };
	...
	/// <summary>Creates a <see cref="ElectricField3D{T}"/> from components in VoltPerMeter.</summary>
	/// <param name="x">The X component, in VoltPerMeter.</param>
	/// <param name="y">The Y component, in VoltPerMeter.</param>
	/// <param name="z">The Z component, in VoltPerMeter.</param>
	/// <returns>A new <see cref="ElectricField3D{T}"/> storing the SI-base equivalent.</returns>
	public static ElectricField3D<T> FromVoltPerMeter(T x, T y, T z) => new() { X = x, Y = y, Z = z };

	/// <summary>Converts this vector's SI-base components to <paramref name="unit"/>.</summary>
	/// <remarks>Returns bare components rather than a <see cref="ElectricField3D{T}"/>, because a vector that is not in base units cannot be one.</remarks>
	/// <param name="unit">The dimensionally-compatible target unit.</param>
	/// <returns>The components expressed in <paramref name="unit"/>.</returns>
	public (T X, T Y, T Z) In(global::ktsu.Semantics.Quantities.IElectricFieldUnit unit) => (unit.FromBase(X), unit.FromBase(Y), unit.FromBase(Z));
	...
	public T Dot(ElectricField3D<T> other) => (X * other.X) + (Y * other.Y) + (Z * other.Z);

	/// <summary>Calculates the cross product of two vectors.</summary>
	public ElectricField3D<T> Cross(ElectricField3D<T> other)
	{
		return new() { X = (Y * other.Z) - (Z * other.Y), Y = (Z * other.X) - (X * other.Z), Z = (X * other.Y) - (Y * other.X) };
	}

	/// <summary>Calculates the distance between two vectors.</summary>
	public T Distance(ElectricField3D<T> other)
	{
		T dX = X - other.X;
		T dY = Y - other.Y;
		T dZ = Z - other.Z;
	...
	public T DistanceSquared(ElectricField3D<T> other)
	{
		T dX = X - other.X;
		T dY = Y - other.Y;
		T dZ = Z - other.Z;
	...
	public ElectricField3D<T> Normalize()
	{
		T len = Length();
	...
	public static ElectricField3D<T> operator +(ElectricField3D<T> left, ElectricField3D<T> right) => new() { X = left.X + right.X, Y = left.Y + right.Y, Z = left.Z + right.Z };

	/// <summary>Subtracts two vectors.</summary>
	public static ElectricField3D<T> operator -(ElectricField3D<T> left, ElectricField3D<T> right) => new() { X = left.X - right.X, Y = left.Y - right.Y, Z = left.Z - right.Z };

	/// <summary>Multiplies a vector by a scalar.</summary>
	public static ElectricField3D<T> operator *(ElectricField3D<T> vector, T scalar) => new() { X = vector.X * scalar, Y = vector.Y * scalar, Z = vector.Z * scalar };

	/// <summary>Multiplies a scalar by a vector.</summary>
	public static ElectricField3D<T> operator *(T scalar, ElectricField3D<T> vector) => new() { X = scalar * vector.X, Y = scalar * vector.Y, Z = scalar * vector.Z };

	/// <summary>Divides a vector by a scalar.</summary>
	public static ElectricField3D<T> operator /(ElectricField3D<T> vector, T scalar) => new() { X = vector.X / scalar, Y = vector.Y / scalar, Z = vector.Z / scalar };

	/// <summary>Negates a vector.</summary>
	public static ElectricField3D<T> operator -(ElectricField3D<T> vector) => new() { X = -vector.X, Y = -vector.Y, Z = -vector.Z };
};
