...
public readonly partial record struct Jerk4D<T> : IVector4<Jerk4D<T>, T>
	where T : struct, INumber<T>
{
	...
	public static Jerk4D<T> Zero => new() { X = T.Zero, Y = T.Zero, Z = T.Zero, W = T.Zero };

	/// <summary>Gets a vector with all components set to one.</summary>
	public static Jerk4D<T> One => new() { X = T.One, Y = T.One, Z = T.One, W = T.One };

	/// <summary>Gets the unit vector for the X-axis.</summary>
	public static Jerk4D<T> UnitX => new() { X = T.One, Y = T.Zero, Z = T.Zero, W = T.Zero };

	/// <summary>Gets the unit vector for the Y-axis.</summary>
	public static Jerk4D<T> UnitY => new() { X = T.Zero, Y = T.One, Z = T.Zero, W = T.Zero };

	/// <summary>Gets the unit vector for the Z-axis.</summary>
	public static Jerk4D<T> UnitZ => new() { X = T.Zero, Y = T.Zero, Z = T.One, W = T.Zero };
	...
	/// <summary>Creates a <see cref="Jerk4D{T}"/> from components in MeterPerSecondCubed.</summary>
	...
	/// <param name="w">The W component, in MeterPerSecondCubed.</param>
	/// <returns>A new <see cref="Jerk4D{T}"/> storing the SI-base equivalent.</returns>
	public static Jerk4D<T> FromMeterPerSecondCubed(T x, T y, T z, T w) => new() { X = x, Y = y, Z = z, W = w };

	/// <summary>Converts this vector's SI-base components to <paramref name="unit"/>.</summary>
	/// <remarks>Returns bare components rather than a <see cref="Jerk4D{T}"/>, because a vector that is not in base units cannot be one.</remarks>
	/// <param name="unit">The dimensionally-compatible target unit.</param>
	/// <returns>The components expressed in <paramref name="unit"/>.</returns>
	public (T X, T Y, T Z, T W) In(global::ktsu.Semantics.Quantities.IJerkUnit unit) => (unit.FromBase(X), unit.FromBase(Y), unit.FromBase(Z), unit.FromBase(W));
	...
	public T Dot(Jerk4D<T> other) => (X * other.X) + (Y * other.Y) + (Z * other.Z) + (W * other.W);

	/// <summary>Calculates the distance between two vectors.</summary>
	public T Distance(Jerk4D<T> other)
	{
		T dX = X - other.X;
		T dY = Y - other.Y;
		T dZ = Z - other.Z;
	...
	public T DistanceSquared(Jerk4D<T> other)
	{
		T dX = X - other.X;
		T dY = Y - other.Y;
		T dZ = Z - other.Z;
	...
	public static Jerk4D<T> operator +(Jerk4D<T> left, Jerk4D<T> right) => new() { X = left.X + right.X, Y = left.Y + right.Y, Z = left.Z + right.Z, W = left.W + right.W };

	/// <summary>Subtracts two vectors.</summary>
	public static Jerk4D<T> operator -(Jerk4D<T> left, Jerk4D<T> right) => new() { X = left.X - right.X, Y = left.Y - right.Y, Z = left.Z - right.Z, W = left.W - right.W };

	/// <summary>Multiplies a vector by a scalar.</summary>
	public static Jerk4D<T> operator *(Jerk4D<T> vector, T scalar) => new() { X = vector.X * scalar, Y = vector.Y * scalar, Z = vector.Z * scalar, W = vector.W * scalar };

	/// <summary>Multiplies a scalar by a vector.</summary>
	public static Jerk4D<T> operator *(T scalar, Jerk4D<T> vector) => new() { X = scalar * vector.X, Y = scalar * vector.Y, Z = scalar * vector.Z, W = scalar * vector.W };

	/// <summary>Divides a vector by a scalar.</summary>
	public static Jerk4D<T> operator /(Jerk4D<T> vector, T scalar) => new() { X = vector.X / scalar, Y = vector.Y / scalar, Z = vector.Z / scalar, W = vector.W / scalar };

	/// <summary>Negates a vector.</summary>
	public static Jerk4D<T> operator -(Jerk4D<T> vector) => new() { X = -vector.X, Y = -vector.Y, Z = -vector.Z, W = -vector.W };
	/// <summary>Jerk4D * Duration = Acceleration4D.</summary>
	public static Acceleration4D<T> operator *(Jerk4D<T> left, Duration<T> right) => new() { X = left.X * right.Value, Y = left.Y * right.Value, Z = left.Z * right.Value, W = left.W * right.Value };

	/// <summary>Jerk4D / Duration = Snap4D.</summary>
	public static Snap4D<T> operator /(Jerk4D<T> left, Duration<T> right) => new() { X = left.X / right.Value, Y = left.Y / right.Value, Z = left.Z / right.Value, W = left.W / right.Value };
