...
internal class ConstructorTemplate : MemberTemplate
{
	public List<ParameterTemplate> Parameters { get; set; } = [];
	public List<string> BaseParameters { get; set; } = [];
	public Action<CodeBlocker>? BodyFactory { get; set; } = (sw) => { };
	public override void WriteTo(CodeBlocker codeBlocker)
	{
		base.WriteTo(codeBlocker);
		WriteParametersTo(codeBlocker);
		WriteBaseParametersTo(codeBlocker);
	...
	private void WriteBaseParametersTo(CodeBlocker codeBlocker)
	{
		if (BaseParameters.Count == 0)
		{
			return; // No base parameters to write
		}

		codeBlocker.NewLine();
		codeBlocker.Write(": base");
		codeBlocker.Write("(");
		codeBlocker.Write(string.Join(", ", BaseParameters));
		codeBlocker.Write(")");
	}
