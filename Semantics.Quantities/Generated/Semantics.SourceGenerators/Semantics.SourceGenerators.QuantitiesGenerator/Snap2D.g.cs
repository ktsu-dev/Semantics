...
public record Snap2D<T> : IVector2<Snap2D<T>, T>
	where T : struct, INumber<T>
{
	...
	public static Snap2D<T> Zero => new() { X = T.Zero, Y = T.Zero };

	/// <summary>Gets a vector with all components set to one.</summary>
	public static Snap2D<T> One => new() { X = T.One, Y = T.One };

	/// <summary>Gets the unit vector for the X-axis.</summary>
	public static Snap2D<T> UnitX => new() { X = T.One, Y = T.Zero };
	...
	public T Dot(Snap2D<T> other) => (X * other.X) + (Y * other.Y);

	/// <summary>Calculates the distance between two vectors.</summary>
	public T Distance(Snap2D<T> other)
	{
		T dX = X - other.X;
		T dY = Y - other.Y;
		T sum = (dX * dX) + (dY * dY);
	...
	public T DistanceSquared(Snap2D<T> other)
	{
		T dX = X - other.X;
		T dY = Y - other.Y;
		return (dX * dX) + (dY * dY);
	...
	public static Snap2D<T> operator +(Snap2D<T> left, Snap2D<T> right) => new() { X = left.X + right.X, Y = left.Y + right.Y };

	/// <summary>Subtracts two vectors.</summary>
	public static Snap2D<T> operator -(Snap2D<T> left, Snap2D<T> right) => new() { X = left.X - right.X, Y = left.Y - right.Y };

	/// <summary>Multiplies a vector by a scalar.</summary>
	public static Snap2D<T> operator *(Snap2D<T> vector, T scalar) => new() { X = vector.X * scalar, Y = vector.Y * scalar };

	/// <summary>Multiplies a scalar by a vector.</summary>
	public static Snap2D<T> operator *(T scalar, Snap2D<T> vector) => new() { X = scalar * vector.X, Y = scalar * vector.Y };

	/// <summary>Divides a vector by a scalar.</summary>
	public static Snap2D<T> operator /(Snap2D<T> vector, T scalar) => new() { X = vector.X / scalar, Y = vector.Y / scalar };

	/// <summary>Negates a vector.</summary>
	public static Snap2D<T> operator -(Snap2D<T> vector) => new() { X = -vector.X, Y = -vector.Y };
	/// <summary>Snap2D * Duration = Jerk2D.</summary>
	public static Jerk2D<T> operator *(Snap2D<T> left, Duration<T> right) => new() { X = left.X * right.Value, Y = left.Y * right.Value };
