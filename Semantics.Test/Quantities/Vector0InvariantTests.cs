...
public sealed class Vector0InvariantTests
{
	...
	[TestMethod]
	public void Speed_FromMeterPerSecond_Negative_Throws()
		=> _ = Assert.ThrowsExactly<ArgumentException>(() => Speed<double>.FromMeterPerSecond(-1.0));
	...
	[TestMethod]
	public void Speed_FromMeterPerSecond_Zero_Allowed()
	{
		Speed<double> s = Speed<double>.FromMeterPerSecond(0.0);
	...
	[TestMethod]
	public void Velocity1D_FromMeterPerSecond_Negative_Allowed()
	{
	...
	{
		Speed<double> a = Speed<double>.FromMeterPerSecond(20.0);
		Speed<double> b = Speed<double>.FromMeterPerSecond(50.0);
	...
	public void Wavelength_FromMeters_Zero_Throws()
		=> _ = Assert.ThrowsExactly<ArgumentException>(() => Wavelength<double>.FromMeter(0.0));

	[TestMethod]
	public void Wavelength_FromMeters_Negative_Throws()
		=> _ = Assert.ThrowsExactly<ArgumentException>(() => Wavelength<double>.FromMeter(-1e-9));

	[TestMethod]
	public void Wavelength_FromMeters_Positive_Succeeds()
	{
		Wavelength<double> w = Wavelength<double>.FromMeter(550e-9);
		Assert.AreEqual(550e-9, w.Value, 1e-15);
	}

	[TestMethod]
	public void Wavelength_FromNanometers_Zero_Throws()
		=> _ = Assert.ThrowsExactly<ArgumentException>(() => Wavelength<double>.FromNanometer(0.0));

	[TestMethod]
	public void Period_FromSeconds_Zero_Throws()
		=> _ = Assert.ThrowsExactly<ArgumentException>(() => Period<double>.FromSecond(0.0));

	[TestMethod]
	public void Period_FromSeconds_Positive_Succeeds()
	{
		Period<double> p = Period<double>.FromSecond(0.001);
		Assert.AreEqual(0.001, p.Value, Tolerance);
	}

	[TestMethod]
	public void HalfLife_FromSeconds_Zero_Throws()
		=> _ = Assert.ThrowsExactly<ArgumentException>(() => HalfLife<double>.FromSecond(0.0));
	...
	{
		Length<double> l = Length<double>.FromMeter(0.0);
	...
	{
		Duration<double> d = Duration<double>.FromSecond(0.0);
	...
	{
		Distance<double> d = Distance<double>.FromMeter(0.0);
		Assert.AreEqual(0.0, d.Value, Tolerance);
	}
	...
	public void Latency_FromSeconds_Zero_Allowed()
	{
		...
		Latency<double> l = Latency<double>.FromSecond(0.0);
		Assert.AreEqual(0.0, l.Value, Tolerance);
	}
