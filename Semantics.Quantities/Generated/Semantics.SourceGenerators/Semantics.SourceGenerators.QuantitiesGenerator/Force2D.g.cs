...
public readonly partial record struct Force2D<T> : IVector2<Force2D<T>, T>
	where T : struct, INumber<T>
{
	...
	/// <summary>Creates a <see cref="Force2D{T}"/> from components in Newton.</summary>
	/// <param name="x">The X component, in Newton.</param>
	/// <param name="y">The Y component, in Newton.</param>
	/// <returns>A new <see cref="Force2D{T}"/> storing the SI-base equivalent.</returns>
	public static Force2D<T> FromNewton(T x, T y) => new() { X = x, Y = y };

	/// <summary>Creates a <see cref="Force2D{T}"/> from components in Kilonewton.</summary>
	/// <param name="x">The X component, in Kilonewton.</param>
	/// <param name="y">The Y component, in Kilonewton.</param>
	/// <returns>A new <see cref="Force2D{T}"/> storing the SI-base equivalent.</returns>
	public static Force2D<T> FromKilonewton(T x, T y) => new() { X = (x * MetricMagnitudes.Values<T>.Kilo), Y = (y * MetricMagnitudes.Values<T>.Kilo) };

	/// <summary>Creates a <see cref="Force2D{T}"/> from components in Dyne.</summary>
	/// <param name="x">The X component, in Dyne.</param>
	/// <param name="y">The Y component, in Dyne.</param>
	/// <returns>A new <see cref="Force2D{T}"/> storing the SI-base equivalent.</returns>
	public static Force2D<T> FromDyne(T x, T y) => new() { X = (x * Units.ConversionConstants.Values<T>.DyneToNewtons), Y = (y * Units.ConversionConstants.Values<T>.DyneToNewtons) };

	/// <summary>Creates a <see cref="Force2D{T}"/> from components in PoundForce.</summary>
	/// <param name="x">The X component, in PoundForce.</param>
	/// <param name="y">The Y component, in PoundForce.</param>
	/// <returns>A new <see cref="Force2D{T}"/> storing the SI-base equivalent.</returns>
	public static Force2D<T> FromPoundForce(T x, T y) => new() { X = (x * Units.ConversionConstants.Values<T>.PoundForceToNewtons), Y = (y * Units.ConversionConstants.Values<T>.PoundForceToNewtons) };

	/// <summary>Converts this vector's SI-base components to <paramref name="unit"/>.</summary>
	/// <remarks>Returns bare components rather than a <see cref="Force2D{T}"/>, because a vector that is not in base units cannot be one.</remarks>
	/// <param name="unit">The dimensionally-compatible target unit.</param>
	/// <returns>The components expressed in <paramref name="unit"/>.</returns>
	public (T X, T Y) In(global::ktsu.Semantics.Quantities.IForceUnit unit) => (unit.FromBase(X), unit.FromBase(Y));
