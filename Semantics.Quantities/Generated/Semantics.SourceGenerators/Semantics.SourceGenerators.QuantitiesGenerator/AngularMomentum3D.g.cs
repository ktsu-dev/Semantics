...
public readonly partial record struct AngularMomentum3D<T> : IVector3<AngularMomentum3D<T>, T>
	where T : struct, INumber<T>
{
	...
	public static AngularMomentum3D<T> Zero => new() { X = T.Zero, Y = T.Zero, Z = T.Zero };

	/// <summary>Gets a vector with all components set to one.</summary>
	public static AngularMomentum3D<T> One => new() { X = T.One, Y = T.One, Z = T.One };

	/// <summary>Gets the unit vector for the X-axis.</summary>
	public static AngularMomentum3D<T> UnitX => new() { X = T.One, Y = T.Zero, Z = T.Zero };

	/// <summary>Gets the unit vector for the Y-axis.</summary>
	public static AngularMomentum3D<T> UnitY => new() { X = T.Zero, Y = T.One, Z = T.Zero };
	...
	public T Dot(AngularMomentum3D<T> other) => (X * other.X) + (Y * other.Y) + (Z * other.Z);

	/// <summary>Calculates the cross product of two vectors.</summary>
	public AngularMomentum3D<T> Cross(AngularMomentum3D<T> other)
	{
		return new() { X = (Y * other.Z) - (Z * other.Y), Y = (Z * other.X) - (X * other.Z), Z = (X * other.Y) - (Y * other.X) };
	}

	/// <summary>Calculates the distance between two vectors.</summary>
	public T Distance(AngularMomentum3D<T> other)
	{
		T dX = X - other.X;
		T dY = Y - other.Y;
		T dZ = Z - other.Z;
	...
	public T DistanceSquared(AngularMomentum3D<T> other)
	{
		T dX = X - other.X;
		T dY = Y - other.Y;
		T dZ = Z - other.Z;
	...
	public AngularMomentum3D<T> Normalize()
	{
		T len = Length();
	...
	public static AngularMomentum3D<T> operator +(AngularMomentum3D<T> left, AngularMomentum3D<T> right) => new() { X = left.X + right.X, Y = left.Y + right.Y, Z = left.Z + right.Z };

	/// <summary>Subtracts two vectors.</summary>
	public static AngularMomentum3D<T> operator -(AngularMomentum3D<T> left, AngularMomentum3D<T> right) => new() { X = left.X - right.X, Y = left.Y - right.Y, Z = left.Z - right.Z };

	/// <summary>Multiplies a vector by a scalar.</summary>
	public static AngularMomentum3D<T> operator *(AngularMomentum3D<T> vector, T scalar) => new() { X = vector.X * scalar, Y = vector.Y * scalar, Z = vector.Z * scalar };

	/// <summary>Multiplies a scalar by a vector.</summary>
	public static AngularMomentum3D<T> operator *(T scalar, AngularMomentum3D<T> vector) => new() { X = scalar * vector.X, Y = scalar * vector.Y, Z = scalar * vector.Z };

	/// <summary>Divides a vector by a scalar.</summary>
	public static AngularMomentum3D<T> operator /(AngularMomentum3D<T> vector, T scalar) => new() { X = vector.X / scalar, Y = vector.Y / scalar, Z = vector.Z / scalar };

	/// <summary>Negates a vector.</summary>
	public static AngularMomentum3D<T> operator -(AngularMomentum3D<T> vector) => new() { X = -vector.X, Y = -vector.Y, Z = -vector.Z };
	/// <summary>AngularMomentum3D / Duration = Torque3D.</summary>
	public static Torque3D<T> operator /(AngularMomentum3D<T> left, Duration<T> right) => new() { X = left.X / right.Value, Y = left.Y / right.Value, Z = left.Z / right.Value };
